...
const DeleteItem = ({ item, onDeleteComplete }) => {
    ...
    const handleDelete = async () => {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:8000/api/delete-item/${item.id}/`, {
                method: 'DELETE',
                headers: {
            ...
            onDeleteComplete(true, 'Item deleted successfully'); // Pass success status and message to parent
        } catch (error) {
            console.error(error);
